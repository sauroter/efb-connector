...
`scripts/garmin_fetch.py:_install_profile_tolerance` exists for a related trap. After token auth succeeds, garminconnect fetches the social profile and user settings — neither of which efb-connector reads. Since 0.3.5, `Garmin.login()` catches *any* `GarminConnectAuthenticationError` from that fetch and treats it as "the cached token was rejected": it discards the token and runs a full SSO credential login. `_load_profile_and_settings()` raises that same exception type for a 429 or a 500 as for a genuine 401, so a flaky profile endpoint would burn an SSO login on a perfectly good token on **every** sync — self-reinforcing when the trigger is itself a rate-limit. We therefore swallow **positively-identified transient** failures before `login()` can react — 429, 5xx, and status-less transport errors — and let everything else through so the poisoned-cache recovery still works. Note the cause-less `Invalid profile data found` (endpoint answered three times with a non-dict) is deliberately *not* treated as transient: that is what a session the API tier no longer accepts looks like, and it genuinely wants the re-login.

Two things to keep in mind when touching this:

- **The MFA setup path needs the shim installed explicitly.** `validate_mfa` builds its own client, and since 0.3.5 `resume_login()` runs these same fetches and lets failures propagate. Without it a correct MFA code plus a profile blip raises *past* `client.dump()`, discarding freshly issued tokens so Garmin setup can never complete.
- **Never put a bare HTTP status number in the warning.** `classifyError` (`internal/garmin/python.go`) lowercases the whole stderr blob and checks `"429"` *before* its authentication branch, so a tolerated profile blip would make a later genuine auth failure surface as "Garmin temporarily unavailable" instead of prompting for new credentials. The warning prints `rate-limited` / `server-error` / `transport-error` instead.

When triaging "why is this user hitting SSO every run", check for the `keeping cached token` warning on stderr.
