...
func defaultActivities() []Activity {
	now := time.Now()
	return []Activity{
		{ProviderID: "mock-1", Name: "Morning Paddle", Type: "kayaking", Date: now.Add(-2 * time.Hour), StartTime: now.Add(-2 * time.Hour), DurationSecs: 3600, DistanceM: 5000},
		{ProviderID: "mock-2", Name: "Evening SUP", Type: "sup", Date: now.Add(-26 * time.Hour), StartTime: now.Add(-26 * time.Hour), DurationSecs: 2700, DistanceM: 3500},
		{ProviderID: "mock-3", Name: "Weekend Canoe Trip", Type: "canoeing", Date: now.Add(-50 * time.Hour), StartTime: now.Add(-50 * time.Hour), DurationSecs: 7200, DistanceM: 12000},
	}
}
